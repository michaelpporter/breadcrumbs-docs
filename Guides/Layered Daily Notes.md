...
There are various ways this structure can be achieved. The method in this guide will use the [Templater plugin](https://silentvoid13.github.io/Templater/) to add templates with [[../Explicit-Edge-Builders/Typed Links]] in them to point `up` to the Monthly and Yearly Notes. Then, using Breadcrumbs' [[../Explicit-Edge-Builders/Date Notes]] feature (_different from the core Daily Notes plugin_), we'll point each Daily Note to the note following it (`yesterday/tomorrow`).
...
[[../Explicit-Edge-Builders/Date Notes]] use the date format of your existing Daily Notes to add edges between them in chronological order. To enable this:
...
To set this up, open `Settings > Implied Relations > Transitive`, and add the following [[../Implied-Edge-Builders/Transitive Implied Relations]]:
...
> [!TIP]
> You can also [[../Implied-Edge-Builders/Transitive Implied Relations#Bulk Add Rules|bulk-add]] the rules:
>
> ```
> [month] <- days
> [tomorrow] <- yesterday
