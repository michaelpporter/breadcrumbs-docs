...
> [!INFO]
> In the language of Breadcrumbs, you've just used the [[../Explicit-Edge-Builders/Typed Links|typed-link edge builder]] to add three [[../Explicit-Edge-Builders/Explicit Edge Builders|explicit edges]], each using the `up` [[Edge Fields|edge field]].
...
> [!TIP]
> The `down` edges weren't added manually — Breadcrumbs infers them automatically as the opposite of the `up` edges you added. This is the [[../Implied-Edge-Builders/Implied Edge Builders|implied edges]] system at work.
...
The `up` edges were added explicitly via [[../Explicit-Edge-Builders/Typed Links]]. The `down` edges back from `Music Genres` to each child were added automatically by Breadcrumbs as implied edges.
