...
Rather than opening every task note and adding a `task: [[My Project]]` frontmatter field by hand, use a [[../Explicit-Edge-Builders/List Notes|List Note]] on the project itself. Create your project note and add a bullet list of its tasks:
...
> [!TIP]
> You can nest bullets to model sub-tasks. A nested item will be linked from its parent bullet, not directly from the project note. Use [[../Explicit-Edge-Builders/List Notes#Field Overrides|field overrides]] on individual items if a particular task needs a different field.
...
Keep all your project notes inside a `Projects/` folder. Create an index note in that same folder and mark it as a [[../Explicit-Edge-Builders/Folder Notes|Folder Note]]:
...
Several relationships can be _derived_ rather than typed manually. Open `Settings > Implied Relations > Transitive` and add the following [[../Implied-Edge-Builders/Transitive Implied Relations]]:
...
> [!TIP]
> You can [[../Implied-Edge-Builders/Transitive Implied Relations#Bulk Add Rules|bulk-add]] all three rules at once:
>
> ```
> [task] <- tasks
> [blocks] <- blocked-by
