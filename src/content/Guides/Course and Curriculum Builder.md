...
There are a few ways to build this structure. This guide uses [[../Explicit-Edge-Builders/Dendron Notes]] (or [[../Explicit-Edge-Builders/Regex Notes]] as an alternative) to derive the Course → Module → Lesson hierarchy automatically from your note names. [[../Explicit-Edge-Builders/List Notes]] in each module then add an ordered `next` sequence between lessons. A [[Codeblocks|codeblock tree]] in the top-level Course note renders the full curriculum, and the [[Previous-Next View]] gives learners a persistent navigation bar.
...
If you prefer a different naming convention — for example, `Python - Basics - Variables` with a ` - ` separator — you can use [[../Explicit-Edge-Builders/Regex Notes]] instead of Dendron Notes. Create an index note (e.g. `Python`) and add:
...
The Dendron edge builder gives you the hierarchy, but not the _order_ of lessons within a module. For that, create a [[../Explicit-Edge-Builders/List Notes|List Note]] inside each module.
...
> [!TIP]
> You can [[../Implied-Edge-Builders/Transitive Implied Relations#Bulk Add Rules|bulk-add]] the rule:
>
> ```
> [next-lesson] <- prev-lesson
> ```
