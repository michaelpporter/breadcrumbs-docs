...
- `resolved`: Whether the note is resolved or not (exists as a markdown file). An _unresolved_ node is referenced in an edge but has no corresponding `.md` file in the vault. Some builders (e.g. [[Explicit-Edge-Builders/Date Notes|date_note]]'s "stretch to existing" option) behave differently for unresolved nodes.
...
- `explicit`: Whether the edge is [[Explicit-Edge-Builders/Explicit Edge Builders|explicit]] or [[Implied-Edge-Builders/Implied Edge Builders|implied]]
- `source`: If the edge is explicit, which [[Explicit-Edge-Builders/Explicit Edge Builders|edge builder]] added it
- `implied_kind`: If the edge is implied, which [[Implied-Edge-Builders/Implied Edge Builders|implied rule]] added it
- `round`: If the edge is implied, which [[Implied-Edge-Builders/Implied Relation Rounds|round]] was it added in
...
- `explicit`: sorts by the explicitness of the edge — explicit edges sort before implied edges
  - Uses `source` as a tiebreaker for [[Explicit-Edge-Builders/Explicit Edge Builders|explicit]] edges, and `implied_kind` for [[Implied-Edge-Builders/Implied Edge Builders|implied]] edges
...
When Breadcrumbs generates [[Implied-Edge-Builders/Implied Edge Builders|implied edges]], it runs in numbered _rounds_. Explicit edges are treated as round `0`. In each subsequent round, the implied relation engine reads the edges produced so far and applies transitive rules to generate new edges, incrementing the round number.
