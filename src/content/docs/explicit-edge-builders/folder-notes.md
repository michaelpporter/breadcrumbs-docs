...
Where `<field>` is one of your [Edge Fields](/edge-fields/). Breadcrumbs will add edges from the current note to all _other_ notes in the same folder, using the field you specify. For example, the following metadata tells Breadcrumbs to point `down` to all _other_ notes in the current folder:
